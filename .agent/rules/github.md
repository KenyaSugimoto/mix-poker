...
### Act（改善）
- 改善点や今後の課題を記録
- 必要ならルールを更新（`.agent/rules/` 配下のファイル）
...
- [ ] `.agent/rules/requirements.md` を確認
...
### 💬 ドキュメントの共有（必須）
以下のタイミングで、ドキュメントの内容をコメントとして投稿してください（手動またはMCPサーバー経由）：

1. **Implementation Plan確定時**
   - 実装計画がユーザーに承認されたら、その内容（`implementation_plan.md`）を**Issueのコメント**に追加してください。
   - PR作成前でも、Issueに計画を記録しておくことで、後からの参照が可能になります。
   - PR作成後は、**PRにも同内容をコメント**してください。

2. **タスク完了（Walkthrough作成時）**
   - 全てのタスクが完了し、`walkthrough.md` を作成・更新したら、その内容を**PRのコメント**に必ず追加してください。
   - 変更の最終的な状態や検証結果を記録し、レビュアーが確認しやすくするためです。

## 🔄 ルール更新フロー

### Issue完了時のルール更新
1. Issueの「Act」フェーズで、得られた知見を整理
2. 必要に応じて `.agent/rules/` 配下のファイルを更新
3. 更新内容をIssueにコメントで記録
4. ルール更新もコミットに含める

### ルール更新の例
```
docs: implementation.mdにZod使用例を追加
